...
class DiagnosisScoreValidation(BaseModel):
    """Validação para scores de diagnóstico"""

    area_key: str = Field(
        ...,
        pattern=r'^(mentalidade|paciente_plano|estrategia|autoridade|captacao|conversao|escala)$',
        description="Chave da área de diagnóstico"
    )
    score: int = Field(
        ...,
        ge=0,
...
class DiagnosisCreate(BaseModel):
    ...
    profile_type: str = Field(
        ...,
        pattern=r'^(iniciante|intermediario|avancado|expert)$',
        description="Tipo de perfil baseado no score"
    )
    area_scores: list[DiagnosisScoreValidation] = Field(
        ...,
        min_length=7,
        max_length=7,
        description="Exatamente 7 áreas de diagnóstico"
    )
